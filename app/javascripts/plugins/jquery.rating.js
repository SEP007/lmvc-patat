...
if (window.jQuery) (function ($) {
    ...
    $.fn.rating.options = { //$.extend($.fn.rating, { options: {
        ...
        callback: function (value) {
            if (this.name.indexOf("restaurant") >= 0) {
                var locId = this.name.substr(18);
                var rating = value.substr(0, 1);
                $.ajax({
                    url: location.origin + "/" + window.lmvcConfig.appDir + "/rating/rateRestaurant/" + locId + "/" + rating,
                    method: 'POST',
                    context: document.body,
                    data: { }
                }).done(function (response) {
                        if (response.location_id == undefined) {
                            alert(response);
                        }
                        ...
                        var divList = $("div[id*='inp_" + locationId + "']");
                        for (var i = 0; i < 5; i++) {
